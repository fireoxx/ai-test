...
#构建web镜像
build-image-web:
	@cd apps/web/ && docker build -t ${REPOSITORY}/web:${TAGS_OPT} .

#构建server镜像
build-image-server:
	@cd apps/server/ && docker build -t ${REPOSITORY}/server:${TAGS_OPT} .
...
build-local:
	...
	&& mkdir build && cp -r apps/web/dist build/ && cp apps/server/server build/ && cp -r apps/server/resource build/resource

#本地环境打包前端
build-web-local:
	@cd apps/web/ && if [ -d "dist" ];then rm -rf dist; else echo "OK!"; fi \
	&& yarn config set registry http://mirrors.cloud.tencent.com/npm/ && yarn install && yarn build

#本地环境打包后端
build-server-local:
	@cd apps/server/ && if [ -f "server" ];then rm -rf server; else echo "OK!"; fi \
...
plugin:
	...
	&& if [ -d "apps/server/plugin/${PLUGIN}" ];then cp -r apps/server/plugin/${PLUGIN} .plugin/${PLUGIN}/server/plugin/ ; else echo "OK!"; fi \
	&& if [ -d "apps/web/src/plugin/${PLUGIN}" ];then cp -r apps/web/src/plugin/${PLUGIN} .plugin/${PLUGIN}/web/plugin/ ; else echo "OK!"; fi \
